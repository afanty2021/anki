# 构建与部署

<cite>
**本文档中引用的文件**  
- [Cargo.toml](file://Cargo.toml)
- [pyproject.toml](file://pyproject.toml)
- [package.json](file://package.json)
- [build.rs](file://rslib/build.rs)
- [hatch_build.py](file://pylib/hatch_build.py)
- [run.py](file://tools/run.py)
- [vite.config.ts](file://ts/vite.config.ts)
- [bundle_ts.mjs](file://ts/bundle_ts.mjs)
- [bundle_svelte.mjs](file://ts/bundle_svelte.mjs)
- [transform_ts.mjs](file://ts/transform_ts.mjs)
- [bench.sh](file://rslib/bench.sh)
- [build.bat](file://tools/build.bat)
- [ninja.bat](file://tools/ninja.bat)
- [runanki.py](file://qt/runanki.py)
- [rsbridge/Cargo.toml](file://pylib/rsbridge/Cargo.toml)
- [launcher/Cargo.toml](file://qt/launcher/Cargo.toml)
- [sync-docs.py](file://tools/sync-docs.py)
- [update-docs.sh](file://tools/update-docs.sh)
- [check-docs.sh](file://tools/check-docs.sh)
- [check-doc-structure.py](file://tools/check-doc-structure.py)
- [check-mermaid-syntax.py](file://tools/check-mermaid-syntax.py)
</cite>

## 更新摘要
**变更内容**   
- 在构建与部署流程中新增了文档同步和验证脚本的集成
- 添加了 CI/CD 流程中的文档质量检查机制
- 更新了核心构建流程以反映新的文档维护实践
- 新增了文档质量验证部分

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心构建流程](#核心构建流程)
4. [Rust核心编译](#rust核心编译)
5. [Python包打包](#python包打包)
6. [前端资源构建](#前端资源构建)
7. [测试策略](#测试策略)
8. [发布流程](#发布流程)
9. [构建环境设置指南](#构建环境设置指南)
10. [高级构建优化技巧](#高级构建优化技巧)
11. [文档质量验证](#文档质量验证)
12. [结论](#结论)

## 简介
Anki 是一个跨平台的记忆卡片应用，其构建系统融合了 Rust、Python 和现代前端技术栈。本文件详细说明 Anki 的构建与部署流程，涵盖从源码编译到最终发布的完整生命周期。文档面向不同层次的开发者，既为初学者提供环境搭建指导，也为高级用户提供性能优化建议。

## 项目结构
Anki 项目采用多语言混合架构，主要分为以下几个核心模块：

```mermaid
graph TD
A[Anki 项目] --> B[Rust 核心]
A --> C[Python 前端逻辑]
A --> D[TypeScript 前端界面]
A --> E[协议缓冲区定义]
A --> F[国际化资源]
B --> B1[rslib]
B --> B2[ftl]
C --> C1[pylib]
C --> C2[qt]
D --> D1[ts]
E --> E1[proto]
F --> F1[ftl]
```

**Section sources**
- [Cargo.toml](file://Cargo.toml)
- [pyproject.toml](file://pylib/pyproject.toml)
- [package.json](file://package.json)

## 核心构建流程
Anki 的构建流程是一个多阶段的协同过程，涉及三种主要技术栈的集成。现在，该流程还包括文档同步和验证步骤，确保代码变更后文档能够及时更新。

```mermaid
flowchart TD
Start([开始构建]) --> RustBuild["编译 Rust 核心 (rslib)"]
RustBuild --> PythonBuild["打包 Python 模块 (pylib)"]
PythonBuild --> FrontendBuild["构建前端资源 (ts)"]
FrontendBuild --> Integration["集成所有组件"]
Integration --> Test["运行测试套件"]
Test --> DocSync["同步和验证文档"]
DocSync --> Package["创建分发包"]
Package --> End([构建完成])
style Start fill:#f9f,stroke:#333
style End fill:#bbf,stroke:#333
```

**Diagram sources**
- [rslib/build.rs](file://rslib/build.rs)
- [pylib/hatch_build.py](file://pylib/hatch_build.py)
- [ts/vite.config.ts](file://ts/vite.config.ts)
- [tools/update-docs.sh](file://tools/update-docs.sh)
- [tools/check-docs.sh](file://tools/check-docs.sh)

**Section sources**
- [rslib/build.rs](file://rslib/build.rs)
- [pylib/hatch_build.py](file://pylib/hatch_build.py)
- [ts/vite.config.ts](file://ts/vite.config.ts)

## Rust核心编译
Rust 核心是 Anki 的性能关键部分，位于 `rslib` 目录中，使用 Cargo 进行管理。

### 编译机制
Rust 核心通过 `build.rs` 构建脚本进行编译，该脚本负责生成必要的绑定和配置。

```mermaid
sequenceDiagram
participant BuildScript as build.rs
participant Cargo as Cargo
participant PythonBridge as rsbridge
Cargo->>BuildScript : 执行构建脚本
BuildScript->>BuildScript : 生成 FFI 绑定
BuildScript->>BuildScript : 编译 Rust 库
BuildScript->>PythonBridge : 生成 Python 可调用接口
PythonBridge-->>Cargo : 返回编译结果
Cargo-->>用户 : 输出静态库/动态库
```

**Diagram sources**
- [rslib/build.rs](file://rslib/build.rs)
- [pylib/rsbridge/build.rs](file://pylib/rsbridge/build.rs)
- [rslib/Cargo.toml](file://rslib/Cargo.toml)

**Section sources**
- [rslib/build.rs](file://rslib/build.rs)
- [rslib/Cargo.toml](file://rslib/Cargo.toml)

## Python包打包
Python 层负责应用程序的主要逻辑和 Qt 界面集成，使用 Hatch 进行打包。

### 打包流程
Python 模块的打包由 `hatch_build.py` 脚本控制，确保正确包含 Rust 编译产物。

```mermaid
flowchart TD
A[开始打包] --> B["读取 pyproject.toml"]
B --> C["执行 hatch_build.py"]
C --> D["包含 Rust 编译产物"]
D --> E["生成 Python 包元数据"]
E --> F["创建 wheel 文件"]
F --> G[打包完成]
style A fill:#f9f,stroke:#333
style G fill:#bbf,stroke:#333
```

**Diagram sources**
- [pylib/hatch_build.py](file://pylib/hatch_build.py)
- [pylib/pyproject.toml](file://pylib/pyproject.toml)
- [qt/hatch_build.py](file://qt/hatch_build.py)

**Section sources**
- [pylib/hatch_build.py](file://pylib/hatch_build.py)
- [pylib/pyproject.toml](file://pylib/pyproject.toml)

## 前端资源构建
前端界面使用现代 JavaScript 工具链构建，主要涉及 TypeScript 和 Svelte 组件。

### 构建管道
前端资源通过 Vite 和自定义脚本进行构建和打包。

```mermaid
flowchart TD
A[源码] --> B["TypeScript 编译 (tsc)"]
A --> C["Svelte 编译 (svelte compiler)"]
B --> D["Vite 打包"]
C --> D
D --> E["bundle_ts.mjs 处理"]
E --> F["bundle_svelte.mjs 处理"]
F --> G["transform_ts.mjs 转换"]
G --> H[优化资源]
H --> I[输出到 web 目录]
style A fill:#f9f,stroke:#333
style I fill:#bbf,stroke:#333
```

**Diagram sources**
- [ts/vite.config.ts](file://ts/vite.config.ts)
- [ts/bundle_ts.mjs](file://ts/bundle_ts.mjs)
- [ts/bundle_svelte.mjs](file://ts/bundle_svelte.mjs)
- [ts/transform_ts.mjs](file://ts/transform_ts.mjs)

**Section sources**
- [ts/vite.config.ts](file://ts/vite.config.ts)
- [ts/bundle_ts.mjs](file://ts/bundle_ts.mjs)

## 测试策略
Anki 采用多层次的测试策略，确保代码质量和功能稳定性。

### 测试层次
```mermaid
graph TD
A[测试策略] --> B[单元测试]
A --> C[集成测试]
A --> D[端到端测试]
B --> B1["Rust 单元测试 (cargo test)"]
B --> B2["Python 单元测试 (pytest)"]
B --> B3["TypeScript 单元测试"]
C --> C1["Rust-Python 集成测试"]
C --> C2["前后端集成测试"]
D --> D1["GUI 自动化测试"]
D --> D2["跨平台兼容性测试"]
```

**Diagram sources**
- [rslib/Cargo.toml](file://rslib/Cargo.toml)
- [pylib/tests](file://pylib/tests)
- [qt/tests](file://qt/tests)

**Section sources**
- [rslib/Cargo.toml](file://rslib/Cargo.toml)
- [pylib/tests](file://pylib/tests)

## 发布流程
Anki 的发布流程涉及版本管理、打包和分发机制。

### 发布步骤
```mermaid
flowchart TD
A[版本管理] --> B[创建 Git Tag]
B --> C[运行完整构建]
C --> D[执行所有测试]
D --> E[生成发布包]
E --> F[Windows: build.bat]
E --> G[macOS: build.sh]
E --> H[Linux: 构建脚本]
F --> I[签名和验证]
G --> I
H --> I
I --> J[上传到分发平台]
J --> K[发布完成]
style A fill:#f9f,stroke:#333
style K fill:#bbf,stroke:#333
```

**Diagram sources**
- [tools/build.bat](file://tools/build.bat)
- [qt/launcher/win/build.bat](file://qt/launcher/win/build.bat)
- [qt/mac/build.sh](file://qt/mac/build.sh)

**Section sources**
- [tools/build.bat](file://tools/build.bat)
- [qt/launcher/win/build.bat](file://qt/launcher/win/build.bat)

## 构建环境设置指南
为初学者提供的构建环境设置步骤。

### 基础环境要求
- Rust 工具链 (通过 rust-toolchain.toml 管理)
- Python 3.8+
- Node.js 16+
- Yarn 包管理器
- Qt 开发库

### 设置步骤
1. 克隆仓库
2. 安装 Rust 工具链
3. 安装 Python 依赖
4. 安装 Node.js 依赖
5. 配置环境变量

```bash
# 示例：设置构建环境
git clone https://github.com/ankitects/anki.git
cd anki
rustup override set stable
pip install -r requirements.txt
yarn install
```

**Section sources**
- [rust-toolchain.toml](file://rust-toolchain.toml)
- [package.json](file://package.json)
- [pyproject.toml](file://pyproject.toml)

## 高级构建优化技巧
为经验丰富的开发者提供的构建优化建议。

### 并行构建
利用 Ninja 构建系统实现并行编译：

```mermaid
flowchart LR
A[源文件] --> B[编译器1]
A --> C[编译器2]
A --> D[编译器3]
A --> E[编译器4]
B --> F[目标文件]
C --> F
D --> F
E --> F
F --> G[链接器]
G --> H[最终二进制]
style A fill:#f9f,stroke:#333
style H fill:#bbf,stroke:#333
```

### 缓存策略
- 使用 sccache 加速 Rust 编译
- 启用 pip 缓存
- 配置 yarn 缓存
- 利用构建脚本的增量编译功能

**Section sources**
- [tools/ninja.bat](file://tools/ninja.bat)
- [rslib/bench.sh](file://rslib/bench.sh)
- [tools/run.py](file://tools/run.py)

## 文档质量验证
为了确保文档与代码保持同步，Anki 引入了一套完整的文档同步和验证机制。

### 文档同步工作流程
```mermaid
flowchart TD
A[代码变更] --> B{变更类型检测}
B --> |新增模块| C[创建新模块文档]
B --> |删除模块| D[标记废弃模块]
B --> |重构| E[更新模块结构图]
B --> |API变更| F[更新API文档]
B --> |其他| G{影响评估}
C --> H[更新根文档索引]
D --> H
E --> H
F --> H
G --> |需要更新| H
G --> |不需要| I[结束]
H --> J[运行验证检查]
J --> K[提交变更]
```

**Diagram sources**
- [tools/sync-docs.py](file://tools/sync-docs.py)
- [tools/update-docs.sh](file://tools/update-docs.sh)
- [tools/check-docs.sh](file://tools/check-docs.sh)

### 文档验证工具
Anki 提供了一系列工具来确保文档质量：

- `tools/sync-docs.py`: 检测代码变更并识别需要更新的文档
- `tools/update-docs.sh`: 自动更新文档的脚本
- `tools/check-docs.sh`: 验证文档质量的综合检查工具
- `tools/check-doc-structure.py`: 检查文档结构一致性
- `tools/check-mermaid-syntax.py`: 验证 Mermaid 图表语法

**Section sources**
- [tools/sync-docs.py](file://tools/sync-docs.py)
- [tools/update-docs.sh](file://tools/update-docs.sh)
- [tools/check-docs.sh](file://tools/check-docs.sh)
- [tools/check-doc-structure.py](file://tools/check-doc-structure.py)
- [tools/check-mermaid-syntax.py](file://tools/check-mermaid-syntax.py)

### CI/CD 集成
文档质量检查已集成到 CI/CD 流程中，确保每次代码提交都经过文档验证：

```yaml
name: Documentation Checks

on:
  pull_request:
    paths:
      - '**/CLAUDE.md'
  push:
    branches:
      - main
    paths:
      - '**/CLAUDE.md'

jobs:
  doc-quality:
    runs-on: ubuntu-latest
    steps:
      - uses: actions/checkout@v3

      - name: Setup Python
        uses: actions/setup-python@v4
        with:
          python-version: '3.9'

      - name: Install dependencies
        run: |
          pip install markdownlint-cli markdown-link-check

      - name: Check documentation quality
        run: |
          ./tools/check-docs.sh

      - name: Check if docs need update
        run: |
          python3 tools/sync-docs.py --ci-mode
```

**Section sources**
- [tools/check-docs.sh](file://tools/check-docs.sh)
- [tools/sync-docs.py](file://tools/sync-docs.py)

## 结论
Anki 的构建与部署系统是一个复杂的多语言集成系统，现在还包括了完善的文档同步和验证机制。通过将文档质量检查集成到 CI/CD 流程中，确保了代码变更后文档能够及时、准确地更新，从而提高了项目的可维护性和开发效率。